...
function buildEntryRoleDescriptor(entry) {
  return normalizeSearchText([entry.label, entry.summary, entry.groupName, entry.comment, ...entry.tags].join(" "));
}
function isLikelyEntityLabel(label) {
  const rawTokens = label.trim().split(/\s+/).map((token) => token.replace(/[^A-Za-z0-9'_-]/g, "")).filter(Boolean);
  if (!rawTokens.length || rawTokens.length > 3)
    return false;
  const normalized = normalizeSearchText(label);
  if (matchesDescriptorTerm(normalized, GROUP_TERMS) || matchesDescriptorTerm(normalized, LOCATION_TERMS) || matchesDescriptorTerm(normalized, EVENT_TERMS) || matchesDescriptorTerm(normalized, THREAT_OR_RULE_TERMS)) {
    return false;
  }
  return rawTokens.every((token) => /^[A-Z][A-Za-z0-9'_-]*$/.test(token));
}
function buildEntityShortVariants(entry) {
  if (!isLikelyEntityLabel(entry.label))
    return [];
  const tokens = normalizeSearchText(entry.label).split(" ").filter(Boolean);
...
function inferSelectionRole(entry, signals) {
  ...
  const personCue = isLikelyEntityLabel(entry.label) && (matchesDescriptorTerm(labelDescriptor, PERSON_TERMS) || matchesDescriptorTerm(commentDescriptor, PERSON_TERMS) || matchesDescriptorTerm(summaryDescriptor, PERSON_TERMS));
  const explicitGroupCue = matchesDescriptorTerm(labelDescriptor, GROUP_TERMS) || matchesDescriptorTerm(commentDescriptor, GROUP_TERMS) || matchesDescriptorTerm(groupNameDescriptor, GROUP_TERMS);
  const summaryGroupSupport = !personCue && matchesDescriptorTerm(summaryDescriptor, GROUP_TERMS) && (summaryDescriptor.includes("act as a group") || summaryDescriptor.includes("serve as a group") || summaryDescriptor.includes("work as a team") || summaryDescriptor.includes("collective") || summaryDescriptor.includes("relationship"));
  const groupCue = explicitGroupCue || summaryGroupSupport;
  const threatCue = matchesDescriptorTerm(labelDescriptor, THREAT_OR_RULE_TERMS) || !personCue && (matchesDescriptorTerm(summaryDescriptor, THREAT_OR_RULE_TERMS) || matchesDescriptorTerm(descriptor, THREAT_OR_RULE_TERMS));
  const eventCue = matchesDescriptorTerm(labelDescriptor, EVENT_TERMS) || !personCue && (matchesDescriptorTerm(summaryDescriptor, EVENT_TERMS) || matchesDescriptorTerm(descriptor, EVENT_TERMS));
  const locationCue = matchesDescriptorTerm(labelDescriptor, LOCATION_TERMS) || !personCue && matchesDescriptorTerm(summaryDescriptor, LOCATION_TERMS);
  const mentionPhrases = buildEntryMentionPhrases(entry, !groupCue && !locationCue && !eventCue && !threatCue);
  const latestMentionCount = mentionPhrases.reduce((total, phrase) => total + countPhraseOccurrences(signals.latestExchange, phrase), 0);
  const overallMentionCount = mentionPhrases.reduce((total, phrase) => total + countPhraseOccurrences(signals.normalizedConversation, phrase), 0);
...
function rankSelectionCandidates(recentConversation, candidates, scopes) {
  ...
  return candidates.map((candidate) => {
    ...
    if (inferred.latestMention)
      priority += 60;
    if (inferred.overallMention)
      priority += 20;
    if (inferred.role === "group_cover" && signals.crowdCue)
      priority += 50;
    if (inferred.role !== "background" && candidate.reasons.includes("label"))
      priority += 10;
    return {
...
function buildDeterministicSelection(rankedCandidates, maxResults) {
  ...
  const results = [];
  const seen = new Set;
  const pushUnique = (items, limit = Number.MAX_SAFE_INTEGER, distinctHelperTypes = false) => {
    const usedHelperTypes = new Set(results.map((item) => item.helperType).filter((value) => !!value));
    for (const item of items) {
      if (results.length >= maxResults || limit <= 0)
        break;
      if (seen.has(item.candidate.entry.entryId))
        continue;
      if (distinctHelperTypes && item.helperType && usedHelperTypes.has(item.helperType))
        continue;
      seen.add(item.candidate.entry.entryId);
      results.push(item);
      if (item.helperType)
        usedHelperTypes.add(item.helperType);
      limit -= 1;
    }
  };
  ...
  const helpers = rankedCandidates.filter((item) => item.selectionRole === "location_context" || item.selectionRole === "event_context" || item.selectionRole === "threat_or_rule_context");
  const entityCount = present.length + mentioned.length;
  const crowdScene = entityCount >= 3 || groups.some((item) => item.latestMention || item.overallMention || item.priority >= 450);
